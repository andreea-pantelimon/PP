...
(define-struct bird_init(y vy))

(define-struct state(bird pipes score))
...
(define-struct score (score))

(define (get-initial-state)
  (make-state (make-bird_init bird-initial-y 0) (list (make-pipe (+ added-number (random random-threshold)) scene-width)) (make-score 0))
  )
...
(define (next-state-bird bird gravity)
  (match-let ([(bird_init y vy) bird])
    (struct-copy bird_init bird [y (+ (bird_init-y bird) (bird_init-vy bird))] [vy (+ (bird_init-vy bird) gravity)]))
  )
...
(define (next-state-bird-onspace bird momentum)
   (match-let ([(bird_init y vy) bird])
    (struct-copy bird_init bird [vy (- (- momentum momentum) momentum)]))
  )
...
(define (change current-state pressed-key)
    (match-let ([(state bird pipe score) current-state])
    (cond [(key=? pressed-key " ")  (struct-copy state current-state [bird (next-state-bird-onspace (state-bird current-state) initial-momentum)])]
          [else state]))
  )
...
(define (move-pipes pipes scroll-speed)
    (list(struct-copy pipe (car pipes) [x (- (pipe-x (car pipes)) scroll-speed)])) 
)
...
(define (clean-pipes pipes)
  (cond
    ((null? pipes) '())
    ((< (+ (pipe-x(car pipes)) pipe-width) 0) (clean-pipes (cdr pipes)))
    (else (cons (car pipes) (clean-pipes (cdr pipes)))))
  )
...
(define (add-more-pipes pipes)
  (cond
    ((< (length pipes) no-pipes) (append pipes (list(make-pipe (+ added-number (random random-threshold)) (+ (+ pipe-width pipe-gap) (if (null? pipes)
                                                                                                                                         0
                                                                                                                                         (pipe-x (car pipes))))))))
...
(define (check-ground-collision bird)
  (cond
    ((>= (- (get-bird-y bird) bird-height) ground-y) #t)
   (else #f))
  )
...
(define (check-pipe-collisions bird pipes)
  (if (or (and (>= (+ bird-x bird-width) (get-pipe-x (car pipes))) (<= (+ bird-x bird-width) (+ (get-pipe-x (car pipes)) pipe-width))) (and (>= bird-x (get-pipe-x (car pipes))) (<= bird-x (+ (get-pipe-x (car pipes)) pipe-width))))
      (if (and (> (bird_init-y bird) (pipe-gap-y (car pipes))) (< (+ (bird_init-y bird) bird-height) (+ (pipe-gap-y (car pipes)) pipe-self-gap)))
          #f
          #t)
      #f)
  )
...
(define (next-state state-a)
   (match-let ([(state bird pipes score) state-a])
    (struct-copy state state-a [bird (next-state-bird bird initial-gravity)]
                 [pipes (next-state-pipes (state-pipes state-a) initial-scroll-speed)]
                 [score (make-score (+ (get-score state-a) 0.1))]))
)
; draw-frame
...
(define (draw-frame state-a)
  (match-let ([(state bird pipes score) state-a])
   (place-image bird-image (+ bird-x (quotient bird-width 2)) (+ (get-bird-y (get-bird state-a)) (quotient bird-height 2))
                 (place-image ground-image (quotient scene-width 2) (+ ground-y (/ ground-height 2))
                              (place-image (score-to-image (get-score state-a)) text-x text-y
                                           (place-image (place-pipes (get-pipes state-a) initial-scene) (quotient pipe-width 2) (quotient pipe-height 2) initial-scene)))))
)
...
(define (place-pipes pipes scene)
  (cond
    ((empty? pipes) scene)
    (else (place-pipes (cdr pipes) (place-pipe (car pipes) scene))))
  )

(define (place-pipe pipe scene)
  (place-top-pipe pipe (place-bottom-pipe pipe scene))
)

(define (place-top-pipe pipe scene)
  (define pipe-image (rectangle pipe-width pipe-height "solid" "green"))
  (place-image/align pipe-image (pipe-x pipe) (+ (pipe-gap-y pipe) (/ pipe-width 2)) "left" "bottom" scene)
)

(define (place-bottom-pipe pipe scene)
  (define pipe-image (rectangle pipe-width pipe-height "solid" "green"))
  (place-image/align pipe-image (pipe-x pipe) (+ (pipe-gap-y pipe) pipe-self-gap) "left" "top" scene)
)
